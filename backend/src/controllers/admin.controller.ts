...
export class AdminController {
	...
	async approveParticipant(req: Request, res: Response): Promise<void> {
		try {
			...
			console.log(
				`📱 Sending approval WhatsApp to: ${updatedParticipant.whatsapp_no}`
			);
			whatsappService
				.sendApprovalMessage(updatedParticipant.whatsapp_no)
				.then((result) => {
					if (result.success) {
						console.log(
							`✅ Approval WhatsApp sent successfully to ${updatedParticipant.whatsapp_no}`
						);
					} else {
						console.error(
							`❌ Failed to send approval WhatsApp to ${updatedParticipant.whatsapp_no}: ${result.error}`
						);
					}
				})
				.catch((error) => {
	...
	async rejectParticipant(req: Request, res: Response): Promise<void> {
		try {
			...
			console.log(
				`📱 Sending rejection WhatsApp to: ${updatedParticipant.whatsapp_no}`
			);
			whatsappService
				.sendRejectionMessage(updatedParticipant.whatsapp_no)
				.then((result) => {
					if (result.success) {
						console.log(
							`✅ Rejection WhatsApp sent successfully to ${updatedParticipant.whatsapp_no}`
						);
					} else {
						console.error(
							`❌ Failed to send rejection WhatsApp to ${updatedParticipant.whatsapp_no}: ${result.error}`
						);
					}
				})
				.catch((error) => {
