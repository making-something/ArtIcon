...
class DatabaseService {
	...
	async getSubmissionByParticipantAndTask(
		...
		const stmt = db.prepare(
			"SELECT * FROM submissions WHERE participant_id = ? AND task_id = ?"
		);
	...
	async getDashboardStats() {
		const totalParticipants = db
			...
			.get() as { count: number };
		const presentParticipants = db
			.prepare(
				"SELECT COUNT(*) as count FROM participants WHERE is_present = 1"
			)
			.get() as { count: number };
	...
	async getWinnerById(
		id: string
	): Promise<(Winner & { participant: Participant }) | null> {
		const stmt = db.prepare(`
      SELECT
        w.*,
        p.id as participant_id, p.name as participant_name, p.email as participant_email,
	...
	}): Promise<Winner & { participant: Participant }> {
		const id = this.generateId();
		const now = new Date().toISOString();
